...
export default function Hospitalities() {
  ...
  const [selectedSeatSection, setSelectedSeatSection] = useState<string>('Tribune d\'Honneur');
  ...
  // Dynamic Metrics
  const totalRevenue = packages.reduce((sum, pkg) => sum + (parsePrice(pkg.price) * (parseInt(pkg.sold) || 0)), 0);
  ...
  const handleEditClick = (pkg: any) => {
    ...
    setTitle(pkg.title);
    setPrice(pkg.price ? pkg.price.replace('€', '') : '150');
            ...
            {!loading && packages.map((pkg, idx) => (
              <div key={pkg.id || idx} className="bg-white border border-slate-150 rounded-xl p-5 hover:border-indigo-300 transition-colors cursor-pointer group flex flex-col shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs font-bold text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg font-mono">{pkg.price}</span>
                </div>
              ...
              <div>
                ...
                <form onSubmit={handleSeatAssign} className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="Tribune d'Honneur">Tribune d'Honneur (Zone A)</option>
                      <option value="Loge Présidentielle">Loge Présidentielle (Zone VIP)</option>
                      <option value="Tribune Nord Elite">Tribune Nord Elite (Zone B)</option>
                      <option value="Prestige Lounge">Prestige Lounge (Makaiaanui Deck)</option>
                    </select>
                  </div>
                  <button
                    type="submit"

  ...
  const handleGoogleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Pointing to public Supabase callback as requested instead of localhost
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 selection:bg-indigo-100 selection:text-indigo-900">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-2 text-center text-3xl font-bold tracking-tight text-slate-900 uppercase">
          {isSignUp ? 'Create Account' : t('login.title', 'System Access')}
        </h2>
        <p className="mt-2 text-center text-sm text-slate-500">
          {isSignUp ? 'Sign up to access the system' : t('login.subtitle', 'Operations Control Center')}
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-10 px-4 shadow-xl shadow-slate-200/50 border border-slate-100 sm:rounded-2xl sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleAuth}>
            ...
            {isSignUp && (
              <div>
                <label className="block text-xs font-bold text-slate-500 tracking-wider uppercase">Confirm Password</label>
                <div className="mt-1">
                  <input
                    type="password"
                    required
            ...
            <div>
              ...
              >
                {loading 
                  ? (isSignUp ? 'Signing up...' : t('auth.loading', 'Authenticating...')) 
                  : (isSignUp ? 'Sign Up' : t('login.submit', 'Sign In'))} 
          ...
          <div className="mt-6 flex flex-col items-center gap-4 text-center">
            <span className="text-xs text-slate-500">
              {isSignUp ? (
                ...
                <>
                  {t('login.noAccount', "Don't have an account?")} <button type="button" onClick={() => { setIsSignUp(true); setError(null); setSuccessMsg(null); }} className="font-bold text-indigo-600 hover:text-indigo-500 transition-colors">Sign Up</button>
                </>
              )}
            </span>
          </div>

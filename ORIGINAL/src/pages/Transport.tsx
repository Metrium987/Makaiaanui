...
export default function Transport() {
  ...
  const handleAssignTransfer = async (driverName: string) => {
    ...
    setActionError(null);
    try {
      // update details or label
      const assignedDetail = `Assigned to ${driverName}`;
      await updateTransfer(showAssignModal.id, {
        time: showAssignModal.time + ' (' + assignedDetail + ')'
      });
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full text-sm border border-slate-200 rounded px-3 py-2 id-input focus:outline-none focus:ring-1 focus:ring-indigo-500 font-sans"
                  />
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl border border-slate-100 max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateTransfer} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 font-sans">Time / Scheduled Slot</label>
                <input 
                  type="text" 
                  value={transferTime}
                  onChange={(e) => setTransferTime(e.target.value)}
                  placeholder="e.g. 14:30 or Immediate"
                  className="w-full text-sm border border-slate-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-sans"
                  required
                />
              </div>

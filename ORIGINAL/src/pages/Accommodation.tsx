...
export default function Accommodation() {
  ...
  // Setup Contract Allotments config (adjustable)
  const [totalAllotments, setTotalAllotments] = useState(120);
  ...
  const hotelCount = uniqueHotels.length || 4; // default minimum is 4 mock/real partner hotels
  ...
  const totalEstimatedRevenue = rooms.filter(r => r.status === 'CONFIRMED' || r.status === 'CHECKED_IN').reduce((sum, r) => sum + getRoomPrice(r.room_type) * 1.15, 0); // Cost + markup as revenue
  ...
  return (
    <div className="flex flex-col gap-6 h-full pb-8">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 uppercase">Hébergement & Allotements</h2>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col flex-1 min-h-[450px]">
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <input 
              type="text" 
              placeholder="Search by name, delegation or hotel..." 
              value={search}

...
import type { Provider, Client, ActivityLog, TransportShiftRow, TransportTransferRow, AccommodationRoomRow, CateringMenu, HospitalityPackageRow, HospitalityGuestRow, AccreditationRow, UniformRow, LaundryRequestRow, AdditionalServiceRow, DeliveryRow } from '../types';
...
export function useTransportShifts() {
  ...
  const fetchShifts = async () => {
    try {
      setLoading(true);
      const query = supabase.from('transport_shifts').select('*').order('created_at', { ascending: false });
...
export function useTransportTransfers() {
  ...
  const fetchTransfers = async () => {
    try {
      setLoading(true);
      const query = supabase.from('transport_transfers').select('*').order('created_at', { ascending: false });
      if (organizationId) query.eq('organization_id', organizationId);
      const { data, error } = await query;
      if (error) throw error;
      setTransfers(data || []);
  ...
  useEffect(() => {
    if (organizationId) fetchTransfers();
  }, [organizationId]);
...
export function useDeliveries() {
  ...
  useEffect(() => {
    if (organizationId) fetchDeliveries();
  }, [organizationId]);
...
export function useAccreditations() {
  ...
  useEffect(() => {
    if (organizationId) fetchAccreditations();
  }, [organizationId]);
...
export function useUniforms() {
  ...
  useEffect(() => {
    if (organizationId) fetchUniforms();
  }, [organizationId]);

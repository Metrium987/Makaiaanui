    ...
    <div className="flex flex-col gap-6 h-full pb-8">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {activeView === 'packages' && (<button onClick={() => { resetForm(); setActionError(null); setShowAddModal(true); }} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-colors flex items-center gap-2 shrink-0"><Plus className="w-4 h-4" />{t('hospitalities.addPackage', 'Add Package')}</button>)}
        </div>
      </div>
        ...
        {activeView === 'packages' && (
          <div className="flex flex-col flex-1 bg-slate-50/50">
            <div className="px-6 py-3 border-b border-slate-100 bg-white flex items-center gap-3">
              <input type="checkbox" checked={isAllSelected} onChange={toggleSelectAll} disabled={loading || packages.length === 0} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
              ...
              {!loading && packages.map((pkg, idx) => (
                <div key={pkg.id || idx} className="bg-white border border-slate-150 rounded-xl p-5 hover:border-indigo-300 transition-colors cursor-pointer group flex flex-col shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      <input type="checkbox" checked={selectedIds.has(pkg.id)} onChange={() => toggleSelect(pkg.id)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" onClick={(e) => e.stopPropagation()} />
                  ...
                  <div className="mt-auto space-y-4">
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                      <button type="button" onClick={(e) => handleQuickSale(pkg, e)} disabled={Number(pkg.sold) >= (Number(pkg.total) || 1)} className="text-xs font-bold uppercase tracking-wider text-emerald-600 hover:text-emerald-700 disabled:text-slate-400 transition-colors flex items-center gap-1"><Ticket className="w-3.5 h-3.5" />{t('hospitalities.quickSale', 'Quick Sale')}</button>
                      <div className="flex items-center gap-1 bg-white shrink-0">
                        <button type="button" onClick={() => handleEditClick(pkg)} className="p-1 text-slate-400 hover:text-indigo-600 transition-colors" title="Edit package parameters"><Edit2 className="w-3.5 h-3.5" /></button>
                        <button type="button" onClick={(e) => handleDeletePackage(pkg.id, e)} className="p-1 text-slate-400 hover:text-red-500 transition-colors" title="Delete Package"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                  </div>
                </div>

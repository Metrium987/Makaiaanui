...
const ALL_SITES_LABEL_KEY = 'deliveries.siteFilterAll';
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm col-span-1 md:col-span-2 flex flex-col justify-between">
          ...
          <div className="mt-4">
            <div className="flex gap-1 h-2 w-full bg-slate-100 rounded-full overflow-hidden">
               ...
               <div className="h-full bg-indigo-500 transition-colors" style={{ width: `${100 - progressPercentage}%` }}></div>
            </div>
                ...
                <tr key={row.id || i} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2.5 py-1 text-[9px] font-bold rounded-lg uppercase tracking-wider border ${
                      ...
                      row.status === 'EN_ROUTE' ? 'bg-blue-50 text-blue-600 border-blue-100 animate-pulse' :
                    ...
                    <div className="flex items-center justify-center gap-3">
                      {row.status !== 'RECEIVED' && (
                        <button
                          ...
                          className="bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-[10px] uppercase font-bold px-2 py-0.5 rounded flex items-center gap-1 font-mono hover:text-indigo-600 transition-colors"

...
export default function AdditionalServices() {
  ...
  const [serviceType, setServiceType] = useState(SERVICE_TYPE_OPTIONS[4].value);
  ...
  const handleEditClick = (svc: any) => {
    ...
    setTitle(svc.title);
    setServiceType(svc.service_type || SERVICE_TYPE_OPTIONS[4].value);
    setPrice(String(svc.price || 0));
    setSoldCount(String(svc.sold_count || 0));
    setLimitCount(svc.limit_count ? String(svc.limit_count) : DEFAULT_LIMIT_COUNT);
    setActionError(null);
  };

  const resetForm = () => {
    setTitle('');
    setServiceType(SERVICE_TYPE_OPTIONS[4].value);
    setPrice(DEFAULT_SERVICE_PRICE);
    setSoldCount('0');
    setLimitCount(DEFAULT_LIMIT_COUNT);
  };

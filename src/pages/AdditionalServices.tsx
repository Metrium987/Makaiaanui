        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left text-sm whitespace-nowrap font-sans">
            <thead className="bg-white border-b border-slate-100 text-[10px] uppercase tracking-widest text-slate-400 font-mono">
              <tr>
                <th className="px-4 py-4 w-10"><input type="checkbox" checked={isAllSelected} onChange={toggleSelectAll} disabled={loading || filteredServices.length === 0} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" /></th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-600 font-sans">
              ...
              {!loading && filteredServices.map((svc, i) => {
                ...
                <tr key={svc.id || i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-4"><input type="checkbox" checked={selectedIds.has(svc.id)} onChange={() => toggleSelect(svc.id)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" /></td>
                  ...
                  <td className="px-6 py-4 text-center"><div className="flex items-center justify-center gap-3"><button type="button" onClick={(e) => handleQuickSale(svc, e)} disabled={isSoldOut} className="bg-indigo-50 border border-indigo-155 text-indigo-700 font-bold px-2 py-0.5 rounded text-[10px] hover:bg-indigo-600 hover:text-white flex items-center gap-1 uppercase tracking-wider disabled:opacity-45 transition-all font-mono" title="Book / buy this service for a client"><ShoppingCart className="w-3 h-3" />{t('additionalServices.bookUnit', 'Book Unit')}</button><button type="button" onClick={() => handleEditClick(svc)} className="p-1 text-slate-400 hover:text-indigo-600 transition-colors" title="Edit catalog price or limit"><Edit2 className="w-3.5 h-3.5" /></button><button type="button" onClick={(e) => handleDelete(svc.id, e)} className="p-1 text-slate-400 hover:text-red-500 transition-colors" title="Delete from catalog"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
                </tr>
              )})}
            </tbody>
          </table>

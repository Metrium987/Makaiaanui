...
export default function Transport() {
  ...
  const handleCreateShift = async (e: React.FormEvent) => {
    ...
    try { await addShift({ driver_name: driverName, vehicle, time: shiftTime, status: shiftStatus, progress: progress + '%' }); setDriverName(''); setVehicle(''); setShiftTime(DEFAULT_SHIFT_TIME); setShiftStatus(TRANSPORT_SHIFT_STATUSES[0].value); setProgress('0'); setShowShiftModal(false); } catch (err: any) { setActionError(err?.message || 'Failed to create shift.'); } finally { setActionLoading(false); }
  };
  ...
  const handleCreateTransfer = async (e: React.FormEvent) => {
    ...
    setActionLoading(true); setActionError(null);
    try { await addTransfer({ time: transferTime, from_location: fromLocation, to_location: toLocation, pax: parseInt(pax) || 1 }); setTransferTime(''); setFromLocation(''); setToLocation(''); setPax(DEFAULT_PAX); setShowTransferModal(false); } catch (err: any) { setActionError(err?.message || 'Failed to create transfer.'); } finally { setActionLoading(false); }
  };

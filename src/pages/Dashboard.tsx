      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0">
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-slate-900">{isLoading ? t('dashboard.connecting', 'Connecting...') : t('dashboard.operational', 'All Systems Normal')}</span>
          </div>
          <div className="mt-4 flex items-center justify-between">
            <span className="text-xs text-slate-500 italic">{appName}</span>
            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${isLoading ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex-1 p-0 divide-y divide-slate-100 min-h-[200px]">
            {logsLoading ? (
              <div className="p-4 text-center text-sm text-slate-500">{t('dashboard.loading', 'Loading activity...')}</div>
            ) : logs.length > 0 ? (
              logs.map((act) => (
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex-1 p-0 divide-y divide-slate-100 min-h-[200px] overflow-y-auto">
            {deliveriesLoading ? (
              <div className="p-4 text-center text-sm text-slate-500">{t('dashboard.loading', 'Loading...')}</div>
